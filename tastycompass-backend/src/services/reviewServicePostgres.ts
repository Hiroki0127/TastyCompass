...
import { Review, CreateReviewRequest, UpdateReviewRequest, ReviewsResponse, ReviewStats } from '../types/review';
import { UserService } from './userServicePostgres';
...
export class ReviewService {
  async createReview(userId: string, reviewData: CreateReviewRequest): Promise<Review> {
    ...
    try {
      ...
      const existingReview = await client.query(
        ...
        [userId, reviewData.restaurantId]
      );
      ...
      await client.query(
        `INSERT INTO reviews (id, user_id, restaurant_id, rating, title, content, helpful_count, is_reported, created_at, updated_at)
  ...
  async updateReview(userId: string, reviewId: string, updateData: UpdateReviewRequest): Promise<Review> {
    ...
    try {
      ...
      await client.query(
        `UPDATE reviews SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${paramIndex}`,
        values
      );
  ...
  async getUserReview(userId: string, restaurantId: string): Promise<Review | null> {
    ...
    try {
      const result = await client.query(
        'SELECT r.*, u.name as user_name FROM reviews r JOIN users u ON r.user_id = u.id WHERE r.user_id = $1 AND r.restaurant_id = $2',
        [userId, restaurantId]
      );
  ...
  async getReviewsForRestaurant(restaurantId: string, limit: number = 10, offset: number = 0): Promise<ReviewsResponse> {
    const client = await pool.connect();
    
    try {
      // Get total count
      const countResult = await client.query(
        'SELECT COUNT(*) as total FROM reviews WHERE restaurant_id = $1 AND is_reported = false',
        [restaurantId]
      );
      ...
      const avgResult = await client.query(
        'SELECT AVG(rating) as avg_rating FROM reviews WHERE restaurant_id = $1 AND is_reported = false',
        [restaurantId]
      );
  ...
  async getUserReviews(userId: string, limit: number = 20, offset: number = 0): Promise<Review[]> {
    const client = await pool.connect();
    ...
    try {
      const result = await client.query(
        `SELECT 
         ...
         FROM reviews 
         WHERE restaurant_id = $1 AND is_reported = false`,
        [restaurantId]
      );
